    ...
    useEffect(() => {
        d3.json('jsontest_2.json').then(data => {
            ...
            const update = (data) => {
                ...
                const nodes =  data.copy().count().value
                let height = nodes/3 + 1000
                let width = nodes/3 + 1000
                let radius = width/2 - 50

                let tree = d3.tree()
                .size([2 * Math.PI, radius]) 
                .separation((a, b) => (a.parent == b.parent ? 1 : 12)/ a.depth)
            ...
            function collapseLevel(event,d,data){
                console.log(data)
                if (d.children) {
